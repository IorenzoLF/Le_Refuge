...
🔄 MIGRÉ depuis spheres/collection.py
Module essentiel de gestion des collections de sphères avec persistance.
"""
...
# 🔧 CORRIGÉ: Import depuis la structure actuelle
...
class SphereCollection:
    ...
    def est_active(self) -> bool:
        """Vérifie si la sphère est active."""
    ...
    def accueillir_sphere_cerisier(self, nom_sphere: str) -> bool:
        ...
        try:
            if hasattr(TypeSphere, nom_sphere):
                type_sphere = getattr(TypeSphere, nom_sphere)
                if type_sphere in self.spheres:
                    # Augmenter la luminosité de la sphère accueillie
                    self.spheres[type_sphere].luminosite = min(1.0, self.spheres[type_sphere].luminosite + 0.1)
                    # logger.info(f"Sphère {nom_sphere} accueillie sous le cerisier")  # Silencieux pour réduire le bruit
                    return True
            logger.warning(f"Sphère {nom_sphere} non trouvée pour accueil")
            return False
        except Exception as e:
            logger.error(f"Erreur lors de l'accueil de la sphère {nom_sphere}: {e}")
            return False

    def afficher_interface(self):
        """Affiche l'interface des sphères (version non-bloquante)."""
        print("\n🌸 === INTERFACE SPHÈRES DU REFUGE ===")
        print(f"Harmonie globale: {self.harmonie_globale:.2f}")
        print(f"Nombre de sphères: {len(self.spheres)}")
        
        for type_sphere, sphere in self.spheres.items():
            status = "🟢" if sphere.etat == "active" else "🔴"
            print(f"  {status} {type_sphere.value}: {sphere.luminosite:.2f} ({sphere.couleur})")
        
        print("🌸 === FIN INTERFACE SPHÈRES ===\n") 