...
class AnalyseurMusical:
    ...
    def extraire_texte_partition(self, chemin_partition):
        ...
        try:
            ...
            if chemin_partition.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                # Ouvrir l'image
            ...
            elif chemin_partition.lower().endswith(('.txt', '.md')):
                with open(chemin_partition, 'r', encoding='utf-8') as f:
                    texte = f.read()
                return texte
            
            # Pour les autres types de fichiers
            else:
                print(f"⚠️ Type de fichier non supporté: {chemin_partition}")
                return ""
