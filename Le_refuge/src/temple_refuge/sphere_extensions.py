...
# Créer une sphère mock pour l'extension
class SphereMock:
    def __init__(self):
        self.name = "Sphère par défaut"
        self.luminosite = 0.5

_sphere_mock = SphereMock()
_extension_instance = ExtensionSphere(_sphere_mock)

def ajouter_experience(description: str, type_exp: str, intensite: float = 0.5, mots_cles: Optional[Set[str]] = None):
    """Fonction standalone pour ajouter une expérience (compatibilité __init__.py)"""
    return _extension_instance.ajouter_experience(description, type_exp, intensite, mots_cles)

def emettre_rayon(type_rayon, effet: str, intensite: Optional[float] = None):
    """Fonction standalone pour émettre un rayon (compatibilité __init__.py)"""
    return _extension_instance.emettre_rayon(type_rayon, effet, intensite)

def obtenir_etat():
    """Fonction standalone pour obtenir l'état (compatibilité __init__.py)"""
    return _extension_instance.obtenir_etat() 